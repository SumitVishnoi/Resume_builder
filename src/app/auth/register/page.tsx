      ...
      <div className="hidden lg:flex w-[44%] bg-[#7C3AED] flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-6">
          ...
          <p className="text-white/60 text-sm leading-relaxed max-w-xs">
            Create ATS-ready resumes with a live preview editor. Land interviews
            faster — free to get started.
          </p>

          {/* Steps mockup */}
          <div className="mt-8 space-y-3 max-w-xs">
            {[
              { step: "01", label: "Create your account", done: true },
              { step: "02", label: "Fill in your details", done: false },
              { step: "03", label: "Export & apply", done: false },
            ].map(({ step, label, done }) => (
              <div key={step} className="flex items-center gap-3">
                ...
                <span
                  className={`text-sm ${done ? "text-white font-medium" : "text-white/40"}`}
                >
                  {label}
                </span>
              </div>
        ...
        <div className="w-full max-w-[400px] space-y-7">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="block text-[11px] font-semibold text-[#6B7280] uppercase tracking-widest">
                Mobile{" "}
                <span className="text-[#C4C4CC] normal-case font-normal tracking-normal">
                  (optional)
                </span>
              </label>
              <input
            ...
            <div className="space-y-1.5">
              ...
              {password.length > 0 && (
                <div className="flex items-center gap-2 mt-1.5">
                  ...
                  <span className="text-[10px] text-[#9CA3AF] w-12 shrink-0">
                    {password.length < 6
                      ? "Weak"
                      : password.length < 10
                        ? "Fair"
            ...
            <button
              type="submit"
              disabled={!canSubmit}
              className="w-full flex items-center justify-center gap-2 py-3 bg-[#7C3AED] text-white text-sm font-semibold rounded-xl hover:bg-[#6D28D9] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed transition-all mt-1"
            >
              {isLoading ? (
                <>
